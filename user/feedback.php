...
/**
 * User Feedback Page
 * Allows users to submit feedback that is visible to admin
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($rating < 1 || $rating > 5) {
        ...
        $query = "INSERT INTO feedback (user_id, user_name, user_email, user_role, rating, subject, message, status) VALUES (?, ?, ?, 'user', ?, ?, ?, 'pending')";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ississ", $userId, $userName, $userEmail, $rating, $subject, $feedbackMessage);
        ...
        <div class="dashboard-container">
            ...
            <div class="main-content">
                ...
                <div class="welcome-section mb-5 fade-in-up">
                    <h1 class="mb-2">💬 User <span style="background: var(--primary-gradient); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">Feedback</span></h1>
                    <p class="text-muted" style="font-size: 1.1rem; font-weight: 500;">Share your thoughts and help us improve your gym experience.</p>
                </div>
                ...
                <!-- Previous Feedback -->
                <?php
                $prevFeedback = $conn->query("SELECT * FROM feedback WHERE user_id = $userId ORDER BY created_at DESC LIMIT 5")->fetch_all(MYSQLI_ASSOC);
